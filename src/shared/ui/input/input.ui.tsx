...
export function Input({
  ...
  const spanStyles =
    `absolute top-4 z-10 left-3.5 text-dark-gray text-base font-semibold transition-all duration-300 ease-out group`;
  const spanTopStyles = value ? '!top-2 font-normal text-xs': null;
  const inputStyles =
  ...
  return (
    <>
      <Label className={labelStyles}>
        <span className={`${spanStyles} ${spanTopStyles}`}>{label}</span>
        <input
